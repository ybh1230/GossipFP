...
The key idea is not to apply gossip as a generic optimizer. Full-parameter gossip is expensive for segmentation and weakly connected to pixel-level errors. GossipFP instead treats each semantic class as a gossip node. Each node maintains an online feature prototype, builds normalized edges to nearby or confusing classes, and performs multi-round lazy gossip diffusion. The perturbation is not driven by the smoothed state itself, but by the disagreement residual between the original prototype and the diffused consensus state.

![GossipFP framework](docs/gossipfp_framework.svg)

## Why GossipFP

Density-descending perturbation needs an extra normalizing flow model to estimate high-dimensional feature density online. That makes training heavier and the perturbation direction can be statistically meaningful without being semantically targeted. In semi-supervised segmentation, the harder failure mode is usually pseudo-label bias near class boundaries and underrepresented classes. GossipFP addresses this with a compact class graph: row-normalized prototype similarity and teacher confusion define edges, class states diffuse through the graph, and fixed-radius feature perturbations follow the graph-disagreement residual. This makes the method different from ordinary graph smoothing: smoothing produces consensus, while GossipFP uses the residual of consensus as a semantic boundary field.
...
- Fixed-radius perturbation along the gossip residual direction.
...
The argument is the visible GPU id. You can also skip it and use the default GPU `0`. On AutoDL, this means normal single-GPU training; no `torchrun`, no port, and no distributed launcher.
...
`topk` controls how many neighbor classes each class exchanges information with. `temperature` controls the top-K edge softmax. `confusion_weight` mixes normalized prototype similarity and normalized teacher confusion. `min_pixels` is the minimum class support needed to update a prototype. `diffusion_rounds` turns one-hop neighbors into a real gossip diffusion process. `perturbation_mode` is the key ablation switch: `residual` uses the graph-disagreement field `h_c - p_c`, while `consensus` perturbs directly toward the diffused consensus state `h_c`. `loss_weight` is optional and defaults to `0.0`; the main method uses only gossip-guided perturbation consistency.
...
The script also writes a CSV with each class's `||b_c||_2` value. High residual classes indicate semantic boundaries where the class prototype disagrees strongly with its gossip consensus state.

## Suggested Ablations

- Baseline: teacher-student consistency without gossip perturbation.
- Gossip perturbation only: default config.
- Optional separation loss: set `gossip.loss_weight` above `0.0`.
