...
class dec_deeplabv3_plus(nn.Module):
    ...
    def forward(self, x, label, gossip_bank, cfg, eps, adv=False):
        x1, x2, x3, x4 = x
        ...
        if adv:
            assert gossip_bank is not None, "gossip_bank is required for gossip perturbation"
            fts_half_pt = fts_aspp_pt[batch_num // 2 : ]
            pt = attack(fts_half_pt, label, self.classifier, self.final, gossip_bank, cfg, eps)
        ...
        fts_train = fts[:batch_num]
        pred = self.final(fts)
        
        return {'pred': pred, 'fts': fts_clean, 'fts_train': fts_train}
